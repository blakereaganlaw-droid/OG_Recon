...
1. One reconciliation workbook (Section 13) with tabs **Matches**, **Candidate Matches**, **Misdirected**, **Review Notes**.
...
**MET** — required: `trx_id` (= ST transaction number), `amount`, `transaction_date`, `status`, `description`. Used: `deposit_id`, `receipt_id`, `cleared_date`, `offset` (GL), `asset_segments`. Parse the description as `d:{DEPOSIT_ID} | r:{RECEIPT_ID} | {payer/purpose}` (Section 5.4).
...
**P8c — Payer-family receipt-sum group (owner 2026-07-14).** A managed-care remittance lands as ONE positive ACH covering several Receivables receipts from the same payer **family**, deposited the **same day**, whose per-receipt references do not tie the bank line (blank / parked SPN). Group the open Receivables receipts whose counterparty resolves to a payer family the BSL also names (via `payer_family` aliases — e.g. VSHP / Volunteer State Health Plan / TN CARE SELECT / TennCare **==** BlueCare Tennessee / BCBST), restricted to receipts on the **same deposit day** as the BSL (signed lag 0). If the whole same-day same-family group sums **exactly** to the BSL → **Candidate** (`PAYER_FAMILY_GROUP`), never a Match — the addenda's structured payment references may cite other receipts of the same payer, so the reference ambiguity routes it to Candidate. Whole-group sum only; never a blind subset-sum. The same aliases make `payer_contradiction` treat VSHP/TennCare vs BlueCare as agreement, not a contradiction.

**P9 — Payables debit.** A negative BSL vs a single open Payables ST of the same amount, reference-tied → Match; else Candidate.

**Orphan doctrine (owner, 2026-07-19 — `UT_Recon_Forward_Orphan_Doctrine.md`, binding companion).** The open pool has already been worked, sometimes wrongly: it contains orphans (open STs whose money is consumed) and is missing consumed counterparts. Forward incorporation: (R3) the MET↔ST bridge propagates a closed MET status onto the open-looking ST — MET status outranks the ST export; never consume a candidate whose MET row is REC/cleared (runlog `met_status_overrides`). (2.3) Dual-fire twins — available entries sharing one MET `RECEIPT_ID` at equal signed cents keep exactly one available, deterministically (runlog `dual_fire_twins`). (R8) On check rails the check number is the identity: same amount + different check number is a conflict, never a candidate (`_check_number_conflict`; blank references are silence). (Signature #6) A stranded line with no exact-cents counterpart but an ALL-closed MET deposit summing exactly receives an enriched Review naming the deposit and members (`POSSIBLE_AUTO_REC_SPLIT` — run Unreconcile2), never a forced match. R1/R6/R7/R9 were already engine doctrine; R2/R5 (Recon History `Created By`, TCR coverage) activate only when those exports are present.

**PM — Misdirected search (HARD GUARDRAIL, owner 2026-07-18).** The bank deposit can land in ONE account while the system transaction / receipt is booked to a DIFFERENT bank account — any source (AR/AP/EXT), any bank (FHB/Regions), any campus/institute/department (real case: City of Memphis $70,992.66 landed in FHB UTHSC while receipt 300045836 remits to FHB Master — "the ST does not exist" on UTHSC). The pool build keeps a **foreign shadow pool**: receipts whose `Remittance Bank Account` maps to another configured account and have no ST in this account's export, and MET rows scoped out to another configured account (bank-name or COA GL key) — `foreign_account` set, `available=False`, never eligible for normal Matches/Candidates. For each still-unplaced BSL, PM searches the shadow pool on **exact signed cents AND a reference tie** (4x4 screen or digit-run; never amount alone; no payer-contradiction screen — a reference tie outranks payer text, e.g. "STATE OF NE" paying for "University of Nebraska"). Hits place on the dedicated **Misdirected** tab (code `MISDIRECTED`), the explanation naming the foreign account and any alternates; the cited entry is consumed (candidate-barred). These lines can never auto-reconcile without a reroute/ECT. PM runs BEFORE P9c — a fully-corroborated foreign hit outranks a partial-group enriched Review.

**P9c — ORT / Receivables 1:M reference search (HARD GUARDRAIL, owner 2026-07-14).** The ORT chain (the bridge/chain for External STs) and the Receivables ST reference columns MUST always be searched for 1:M ties. For each still-unplaced BSL, gather the open non-journal, non-merchant, type-gate-ok, non-stale STs whose **own** reference field — `Reference` or `Transaction Number` — is `znorm`-**exactly equal** (never containment; ≥6 chars, to exclude short date-like coincidences) to one of the BSL's reference fields (`Reference`, `Account Servicer Reference`, recon reference — never the broad addenda/counterparty grid, which is only safe under P4's exact-sum filter). A genuine 1:M deposit batch needs **≥2** such open members, and — because the open members are a SUBSET of the bank total — the group must share the BSL's sign and sum to **strictly less** than it in magnitude (an opposite-sign or oversized group is a coincidental collision on a shared ACH originator / company id, e.g. a `$(1,125)` debit vs 18 positive receipts summing `$66k`, never a partial deposit). Exact-summing groups (with or without an already-closed member) are already placed by P4; only the **partial** remainder reaches here — the open members sum SHORT because the rest of the deposit was already auto-reconciled (stranded) and is absent from the UNR export. The exact-sum invariant (owner 2026-07-11) bars a Match/Candidate whose STs do not sum to the BSL, so P9c emits an **enriched Review** that NAMES the reference-tied open members, quotes the shared reference, the partial sum, and the shortfall, and flags `PARTIAL_REFERENCE_GROUP` / `POSSIBLE_AUTO_REC_SPLIT` (run Unreconcile2 to re-audit and free the stranded members) — rather than a bare "no counterpart" note. It never cites a group that sums exactly (that is P4's), never a single coincidental tie, and never consumes the STs (Review placements do not consume).
...
Every non-Match row carries ≥1 exception code from: `AMBIGUOUS_GROUP, MISSING_MET_LINK, MISSING_REFERENCE, DATE_CONFLICT, PARTIAL_CHAIN, NO_MATCH_FOUND, MULTIPLE_EQUAL_CANDIDATES, DUPLICATE_ST_ATTEMPT, POSSIBLE_AUTO_REC_SPLIT, UNSUPPORTED_TRANSACTION, WEAK_DATE_SUPPORT, INCOMPLETE_REFERENCE_SUPPORT, GROUPING_CONFLICT, INVALID_SOURCE_DATA, RECEIPT_ENTRY_BACKLOG, DUAL_FIRE, DATA_FEED_ERROR, MID_GUARDRAIL, STATE_LANE_ISOLATION, MISDIRECTED`. Backward defects use the codes in Section 10.2.

---

## 13. Output workbook standard (binding)

One reconciliation workbook per account. Tabs: **Matches**, **Candidate Matches**, **Misdirected**, **Review Notes** (Misdirected tab per owner hard guardrail 2026-07-18 — Section 9 pass PM). **HARD GUARDRAIL (owner, 2026-07-11): every row carries ALL BSL identifier fields and ALL ST detail fields — they are essential for reconciliation — and ST lists are NEVER truncated (the ST Number(s) cell lists every cited ST; explanations name every alternate; no ST is ever reused across Matches+Candidates).** Nineteen columns:
...
Formatting: Carlito 11pt; header fill **navy FF1F4E78** with white bold text for the forward reconciliation workbook (dark-red **FF7A1F1F** for the backward/forensic workbook and any reference map); title row 1; row 2 blank; headers row 3; data row 4+; freeze panes at A4; dates as `yyyy-mm-dd` text; USD negatives in parentheses; **static values only — zero formula cells** (prepend a space to any value starting with `=`); multi-value cells use comma-space (semicolon-space when a value embeds a thousands-separator comma), dates in the same order as ST numbers. **Every source BSL appears exactly once across the four tabs. No provenance line, band, comment, or extra row anywhere.**
