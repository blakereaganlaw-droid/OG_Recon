...
| **Award Conversion `.txt` in a per-run upload** | The `run_recon.py` wrapper used to stage only spreadsheets + BAI-token `.txt`, so the Award Conversion `.txt` was silently dropped BEFORE the engine could route it (`GMS_AWARD_CONVERSION`) — while a direct `recon_engine.py` invocation read it fine. The wrapper now mirrors the engine's `.txt` acceptance set (BAI2 by name/content, **and** Award Conversion) and content-sniffs a `BAI Code` CSV, so its manifest role + preflight no longer falsely report `UNROUTED`. | Works as-is now; stage the `Active_Oracle_Award_Conversion_Report.txt` alongside the run. |
